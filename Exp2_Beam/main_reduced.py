"""
Main entry for reduced-order beam PINN (single-branch, no partition).
Provides training snapshots and final visualizations at a level comparable
to the current ADD-PINNs workflow.
"""
import os
...
from reduced_loss import compute_reduced_loss
from reduced_model import ReducedOrderPINN
...
def save_reduced_snapshot(
    model: ReducedOrderPINN,
    *,
    label_path: str,
    out_dir: str,
    suffix,
    ...
    u, theta, kappa, M, V, EI = model(x)
    theta_auto = torch.autograd.grad(u, x, torch.ones_like(u), create_graph=False, retain_graph=True)[0]
    kappa_auto = torch.autograd.grad(theta, x, torch.ones_like(theta), create_graph=False, retain_graph=True)[0]
    M_auto = EI * kappa
    V_auto = torch.autograd.grad(M, x, torch.ones_like(M), create_graph=False, retain_graph=True)[0]
    Q_nn = torch.autograd.grad(V, x, torch.ones_like(V), create_graph=False, retain_graph=True)[0]

    if was_training:
        model.train()

    # No explicit level-set in reduced model; use EI mismatch as proxy to keep phi panel.
    ...
    if plot_panels:
        panels = [
            ...
            ("theta=du/dx", to_np(theta), to_np(theta_auto), arrays["theta_lab"], "theta"),
            ("kappa=d2u/dx2", to_np(kappa), to_np(kappa_auto), arrays["kappa_lab"], "kappa"),
            ("M", to_np(M), to_np(M_auto), arrays["M_lab"], "M"),
            ("V", to_np(V), to_np(V_auto), arrays["V_lab"], "V"),
...
def main() -> ReducedOrderPINN:
    project_root = Path(__file__).resolve().parent
    os.chdir(project_root)
    print(f"Using device: {DEVICE}")
    set_seed()

    model = ReducedOrderPINN().to(DEVICE)
    ...
    loss_rows: List[List[float]] = []
    epoch_offset = 0
    viz_dir = str(project_root / "beam_bechmark_viz_reduced")

    print("\n=== Reduced-order stage 1 ===")
    ...
    out_dir = project_root / "data_output_reduced"
    ...
    print(f"[DONE] reduced loss csv: {loss_path}")
    print(f"[DONE] reduced final outputs dir: {out_dir}")
    return model
