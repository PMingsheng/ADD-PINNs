...
def _dEI_regularization_near_m0(
    ...
    *,
    ...
    dEI = torch.autograd.grad(
        EI, x, grad_outputs=torch.ones_like(EI), create_graph=True, retain_graph=True
    )[0]
    dEI_norm2 = dEI.pow(2)
...
def compute_apinn_beam_loss(
    ...
    *,
    ...
    pred_int = model(x_int_req)
    u = pred_int["u"]
    EI = pred_int["EI"]
    theta = torch.autograd.grad(
        u, x_int_req, grad_outputs=torch.ones_like(u), create_graph=True, retain_graph=True
    )[0]
    kappa = torch.autograd.grad(
        theta,
        x_int_req,
    ...
    V = torch.autograd.grad(
        M, x_int_req, grad_outputs=torch.ones_like(M), create_graph=True, retain_graph=True
    )[0]

    R_theta, R_kappa, R_M, R_V, R_Q = euler_beam_pde(x_int_req, u, theta, kappa, M, V, EI)
    ...
    if x_fit is not None and strain_fit is not None:
        x_fit_req = x_fit.clone().requires_grad_(True)
        pred_fit = model(x_fit_req)
        u_fit_pred = pred_fit["u"]
        theta_fit = torch.autograd.grad(
            u_fit_pred,
            x_fit_req,
            grad_outputs=torch.ones_like(u_fit_pred),
            create_graph=True,
            retain_graph=True,
        )[0]
        kappa_fit = torch.autograd.grad(
            theta_fit,
            x_fit_req,
            grad_outputs=torch.ones_like(theta_fit),
            create_graph=True,
            retain_graph=True,
        )[0]
        strain_pred = (H / 2.0) * kappa_fit
    ...
    w_fai = float(lam.get("fai", 0.0))
    w_dfai = float(lam.get("dfai", 0.0))
