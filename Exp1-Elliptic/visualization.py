...
from level_set import dirac_smooth
...
def local_velocity_viz_cpu(
    ...
    *,
    ...
    mask1 = (phi >= 0).to(phi.dtype)
    mask2 = 1.0 - mask1
    r_val = (mask1 * R1 + mask2 * R2).detach().abs()
    ...
    vel_band = vel_band * dirac_smooth(phi_band)
...
def local_velocity_grad_viz_cpu(
    ...
    *,
    ...
    g_val = (mask_pos * gR1 + mask_neg * gR2).detach()
    phi_det = phi.detach()
...
def local_velocity_CV_full_viz_cpu(
    ...
    *,
    ...
    mask1 = (phi >= 0).to(phi.dtype)
    mask2 = 1.0 - mask1
    R_abs = (mask1 * R1 + mask2 * R2).abs().detach()
...
def _compute_phi_next_on_grid_heat_cpu(
    ...
    u_fit_local: Optional[torch.Tensor],
    get_f1_f2,
    fallback_circles: Optional[Iterable[Tuple[float, float, float]]] = None,
):
    ...
    phi_now, u1_now, u2_now = model_cpu(XY_t)
    ...
    del XY_t, phi_now, u1_now, u2_now, Vn_full, grad_phi, grad_norm, phi_next
    ...
    *,
    ...
    if kind == "data":
        if xy_fit is None or u_fit is None:
            raise ValueError("kind='data' requires xy_fit and u_fit")

        XY_lab_np = xy_fit.detach().cpu().numpy().astype(np.float32)
        u_lab_np = u_fit.detach().cpu().numpy().reshape(-1, 1).astype(np.float32)
        XY_lab_t = torch.from_numpy(XY_lab_np).to(device_cpu)

        with torch.no_grad():
            phi_b, u1_b, u2_b = model_cpu(XY_lab_t)
            mask_pos = (phi_b >= 0).to(phi_b.dtype)
            mask_neg = 1.0 - mask_pos
            u_pred = mask_pos * u1_b + mask_neg * u2_b

        diff = (u_pred.cpu().numpy() - u_lab_np)
        ...
        for i0 in range(0, XY_grid.shape[0], batch_size):
            ...
            phi_b, u1_b, u2_b = model_cpu(XY_b_req)
            f1_b, f2_b = get_f1_f2(XY_b_req)

            R1_b = div_kgrad(u1_b, f1_b, XY_b_req, keep_graph=True)
            R2_b = div_kgrad(u2_b, f2_b, XY_b_req)
            ...
            mask_pos = (phi_b >= 0).to(phi_b.dtype)
            mask_neg = 1.0 - mask_pos
            R_soft = mask_pos * Rmag1 + mask_neg * Rmag2
...
def plot_loss_components_from_records(
    ...
    col_map = dict(total=1, data=2, pde=3, bc=4, interface=5, eik=6, area=7, perimeter=8)
    cols_to_plot = [
        col_map["data"],
        col_map["pde"],
        col_map["interface"],
    ...
    labels = ["Data", "PDE", "Interface", "Eikonal", "Area", "Perimeter", "Total"]
...
def plot_phi_compare_with_flower(
    model,
    *,
    bbox: Tuple[float, float, float, float] = (-1.0, 1.0, -1.0, 1.0),
    n: int = 200,
    ...
    with torch.no_grad():
        phi_true_map = (
            phi_signed_flower(torch.from_numpy(XY_grid).to(device=device, dtype=dtype))
            .detach()
            .cpu()
            .numpy()
            .reshape(n, n)
...
def plot_phi_heatmap(
    ...
    *,
    ...
    ax.contour(Xg, Yg, phi_map, levels=[0.0], colors="red", linewidths=0.9, linestyles="--")
    # Compute IoU between predicted sign mask and true sign mask and annotate
    pred_mask = (phi_map >= 0.0)
    true_mask = (phi_true_map >= 0.0)
    ...
    ax.legend(
        handles=[
            Line2D([0], [0], color="black", lw=1.0, ls="-", label="Exact $\\phi=0$"),
            Line2D([0], [0], color="red", lw=0.9, ls="--", label="Pred $\\phi=0$"),
        ],
        loc="lower right",
        framealpha=0.8,
        edgecolor="none",
    ...
    return {"Xg": Xg, "Yg": Yg, "phi_map": phi_map, "phi_true_map": phi_true_map}


def plot_u_true_pred_residual_heatmap(
    model,
    *,
    bbox: Tuple[float, float, float, float] = (-1.0, 1.0, -1.0, 1.0),
    n: int = 200,
    ...
    u_pred_flat = np.zeros(XY_grid.shape[0], dtype=np.float32)
    for i0 in range(0, XY_grid.shape[0], batch_size):
        i1 = min(XY_grid.shape[0], i0 + batch_size)
        XY_b = torch.from_numpy(XY_grid[i0:i1]).to(device=device, dtype=dtype)
        with torch.no_grad():
            ...
            mask_pos = (phi_b >= 0).to(phi_b.dtype)
            mask_neg = 1.0 - mask_pos
            u_b = mask_pos * u1_b + mask_neg * u2_b
        u_pred_flat[i0:i1] = u_b.detach().cpu().numpy().reshape(-1).astype(np.float32)

    with torch.no_grad():
        u_true_flat = (
            exact_solution(torch.from_numpy(XY_grid).to(device=device, dtype=dtype))
            .detach()
            .cpu()
            .numpy()
            .reshape(-1)
    ...
    u_pred_map = u_pred_flat.reshape(n, n)
    u_true_map = u_true_flat.reshape(n, n)
    u_res_map = np.abs(u_pred_map - u_true_map).astype(np.float32)

    u_vmin = float(min(u_true_map.min(), u_pred_map.min()))
    u_vmax = float(max(u_true_map.max(), u_pred_map.max()))
    if not np.isfinite(u_vmin) or not np.isfinite(u_vmax) or u_vmin == u_vmax:
        u_vmin, u_vmax = -1.0, 1.0

    res_lim = float(np.nanmax(np.abs(u_res_map)))
    if not np.isfinite(res_lim) or res_lim <= 0.0:
        res_lim = 1.0

    fig, axes = plt.subplots(1, 3, dpi=dpi, figsize=(9.6, 3.2), constrained_layout=True)

    im0 = axes[0].contourf(Xg, Yg, u_true_map, levels=60, cmap="viridis", vmin=u_vmin, vmax=u_vmax)
    axes[0].set_title("u true")
    fig.colorbar(im0, ax=axes[0], shrink=0.88)

    im1 = axes[1].contourf(Xg, Yg, u_pred_map, levels=60, cmap="viridis", vmin=u_vmin, vmax=u_vmax)
    ...
    if save_npz_path is not None:
        np.savez_compressed(
            ...
            u_true_map=u_true_map.astype(np.float32),
            u_pred_map=u_pred_map.astype(np.float32),
            u_res_map=u_res_map.astype(np.float32),
            bbox=np.asarray([xmin, xmax, ymin, ymax], dtype=np.float32),
        )
        print("[SAVE]", save_npz_path)
    if show:
    ...
    return {
        "Xg": Xg,
        "Yg": Yg,
        "u_true_map": u_true_map,
        "u_pred_map": u_pred_map,
        "u_res_map": u_res_map,
    }


def plot_f_true_pred_residual_heatmap(
    model,
    *,
    bbox: Tuple[float, float, float, float] = (-1.0, 1.0, -1.0, 1.0),
    n: int = 200,
    ...
    f_true_flat = np.zeros(XY_grid.shape[0], dtype=np.float32)
    f_pred_flat = np.zeros(XY_grid.shape[0], dtype=np.float32)

    for i0 in range(0, XY_grid.shape[0], batch_size):
        i1 = min(XY_grid.shape[0], i0 + batch_size)
        XY_b = torch.from_numpy(XY_grid[i0:i1]).to(device=device, dtype=dtype)
        XY_req = XY_b.clone().requires_grad_(True)

        phi_b, u1_b, u2_b = model_cpu(XY_req)

        f1_true_b = f_region_inside(XY_req).detach()
        f2_true_b = f_region_outside(XY_req).detach()
        phi_true_b = phi_signed_flower(XY_req).detach()
        f_true_b = torch.where(phi_true_b >= 0.0, f1_true_b, f2_true_b)

        zeros = torch.zeros_like(u1_b)
        lap_u1_b = div_kgrad(u1_b, zeros, XY_req, keep_graph=True)
        lap_u2_b = div_kgrad(u2_b, zeros, XY_req)
        f1_pred_b = -lap_u1_b
        f2_pred_b = -lap_u2_b
        ...
        f_true_flat[i0:i1] = f_true_b.detach().cpu().numpy().reshape(-1).astype(np.float32)
        f_pred_flat[i0:i1] = f_pred_b.detach().cpu().numpy().reshape(-1).astype(np.float32)

    f_true_map = f_true_flat.reshape(n, n)
    f_pred_map = f_pred_flat.reshape(n, n)
    f_res_map = np.abs(f_pred_map - f_true_map).astype(np.float32)

    f_vmin = float(min(f_true_map.min(), f_pred_map.min()))
    f_vmax = float(max(f_true_map.max(), f_pred_map.max()))
    if not np.isfinite(f_vmin) or not np.isfinite(f_vmax) or f_vmin == f_vmax:
        f_vmin, f_vmax = -1.0, 1.0

    res_lim = float(np.nanmax(np.abs(f_res_map)))
    if not np.isfinite(res_lim) or res_lim <= 0.0:
        res_lim = 1.0
    ...
    if save_npz_path is not None:
        np.savez_compressed(
            ...
            Xg=Xg.astype(np.float32),
            Yg=Yg.astype(np.float32),
            f_true_map=f_true_map.astype(np.float32),
            f_pred_map=f_pred_map.astype(np.float32),
            f_res_map=f_res_map.astype(np.float32),
            bbox=np.asarray([xmin, xmax, ymin, ymax], dtype=np.float32),
        )
        print("[SAVE]", save_npz_path)
    if show:
    ...
    return {
        "Xg": Xg,
        "Yg": Yg,
        "f_true_map": f_true_map,
        "f_pred_map": f_pred_map,
        "f_res_map": f_res_map,
    }
