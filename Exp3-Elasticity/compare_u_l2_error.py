#!/usr/bin/env python3
from __future__ import annotations

import argparse
import csv
from pathlib import Path
from typing import Dict, List, Optional, Sequence, Tuple

import numpy as np
import pandas as pd
from openpyxl import load_workbook
from openpyxl.utils import get_column_letter
from scipy.interpolate import RegularGridInterpolator

import config
from data import (
    load_ellipse_uv_eps_fit,
    load_ellipse_uv_eps_fit_downsample,
    load_ellipse_uv_eps_fit_rect_dense,
)


def load_reference(txt_filename: Path) -> Tuple[np.ndarray, np.ndarray]:
    with txt_filename.open("r", encoding="utf-8", errors="ignore") as f:
        lines = [ln for ln in f if not ln.lstrip().startswith("%")]
    arr = np.loadtxt(lines)
    xy = arr[:, 0:2].astype(np.float64)
    u = arr[:, 2:4].astype(np.float64)
    return xy, u


def infer_sampling_mode_from_path(path: Path) -> str:
    parts = {part.lower() for part in path.parts}
    if "output_roi_off" in parts or "roi_off" in parts:
        return "roi-off"
    if "output_roi_on" in parts or "roi_on" in parts:
        return "roi-rect"
    return "roi-rect"


def _extract_epoch_from_name(name: str) -> int:
    digits = "".join(ch for ch in name if ch.isdigit())
    return int(digits) if digits else -1


def find_latest_phi_snapshot(snapshot_dir: Path) -> Optional[Path]:
    candidates = list(snapshot_dir.glob("phi_epoch_*.npz"))
    if not candidates:
        return None
    return max(candidates, key=lambda p: (_extract_epoch_from_name(p.name), p.stat().st_mtime))


def _load_axes_and_fields(npz_path: Path) -> Dict[str, np.ndarray]:
    with np.load(npz_path) as zf:
        data = {k: zf[k] for k in zf.files}

    if "x" not in data or "y" not in data:
        raise KeyError(f"{npz_path} missing x/y axes.")

    ux_key = "ux_pred" if "ux_pred" in data else "ux"
    uy_key = "uy_pred" if "uy_pred" in data else "uy"
    if ux_key not in data or uy_key not in data:
        raise KeyError(f"{npz_path} missing ux/uy predictions.")

    epoch = -1
    if "epoch" in data:
        epoch_arr = np.asarray(data["epoch"]).reshape(-1)
        if epoch_arr.size > 0:
            epoch = int(epoch_arr[0])

    if "E_out" in data:
        e_out = float(np.asarray(data["E_out"]).reshape(-1)[0])
    elif "E1" in data:
        e_out = float(np.asarray(data["E1"]).reshape(-1)[0])
    else:
        e_out = float("nan")

    if "E_in" in data:
        e_in = float(np.asarray(data["E_in"]).reshape(-1)[0])
    elif "E2" in data:
        e_in = float(np.asarray(data["E2"]).reshape(-1)[0])
    else:
        e_in = float("nan")

    return {
        "x": np.asarray(data["x"], dtype=np.float64).reshape(-1),
        "y": np.asarray(data["y"], dtype=np.float64).reshape(-1),
        "ux": np.asarray(data[ux_key], dtype=np.float64),
        "uy": np.asarray(data[uy_key], dtype=np.float64),
        "epoch": np.asarray([epoch], dtype=np.int64),
        "E_out": np.asarray([e_out], dtype=np.float64),
        "E_in": np.asarray([e_in], dtype=np.float64),
    }


def sample_prediction_at_points(
    field_map: np.ndarray,
    x_axis: np.ndarray,
    y_axis: np.ndarray,
    xy_points: np.ndarray,
) -> np.ndarray:
    interp = RegularGridInterpolator(
        (np.asarray(x_axis, dtype=np.float64), np.asarray(y_axis, dtype=np.float64)),
        np.asarray(field_map, dtype=np.float64),
        method="linear",
        bounds_error=False,
        fill_value=np.nan,
    )
    return np.asarray(interp(np.asarray(xy_points, dtype=np.float64)), dtype=np.float64).reshape(-1, 1)


def relative_l2(true_values: np.ndarray, pred_values: np.ndarray) -> float:
    diff = np.asarray(pred_values, dtype=np.float64).reshape(-1) - np.asarray(true_values, dtype=np.float64).reshape(-1)
    denom = np.linalg.norm(np.asarray(true_values, dtype=np.float64).reshape(-1), ord=2) + 1e-15
    return float(np.linalg.norm(diff, ord=2) / denom)


def pointwise_relative_percentages(
    true_values: np.ndarray,
    pred_values: np.ndarray,
    thresholds: Sequence[float],
    *,
    eps: float = 1e-12,
) -> Dict[float, float]:
    true_arr = np.asarray(true_values, dtype=np.float64)
    pred_arr = np.asarray(pred_values, dtype=np.float64)
    if true_arr.ndim == 1 or (true_arr.ndim == 2 and true_arr.shape[1] == 1):
        num = np.abs(pred_arr.reshape(-1) - true_arr.reshape(-1))
        den = np.maximum(np.abs(true_arr.reshape(-1)), eps)
    else:
        num = np.linalg.norm(pred_arr - true_arr, axis=1)
        den = np.maximum(np.linalg.norm(true_arr, axis=1), eps)
    rel = num / den
    return {thr: float(np.mean(rel <= thr) * 100.0) for thr in thresholds}


def format_display(value: float, digits: int = 2) -> str:
    value = float(value)
    if not np.isfinite(value):
        return str(value)
    return f"{value:.{digits}E}"


def format_percent(value: float, digits: int = 2) -> str:
    value = float(value)
    if not np.isfinite(value):
        return str(value)
    return f"{value:.{digits}f}%"


def threshold_column_name(threshold: float) -> str:
    pct = threshold * 100.0
    if abs(pct - round(pct)) < 1e-12:
        pct_str = str(int(round(pct)))
    else:
        pct_str = f"{pct:.3f}".rstrip("0").rstrip(".")
    return f"points_within_{pct_str}pct"


def threshold_column_label(threshold: float) -> str:
    pct = threshold * 100.0
    if abs(pct - round(pct)) < 1e-12:
        pct_str = str(int(round(pct)))
    else:
        pct_str = f"{pct:.3f}".rstrip("0").rstrip(".")
    return f"Points within {pct_str}% Error (%)"


def threshold_header(threshold: float) -> str:
    pct = threshold * 100.0
    if abs(pct - round(pct)) < 1e-12:
        pct_str = str(int(round(pct)))
    else:
        pct_str = f"{pct:.3f}".rstrip("0").rstrip(".")
    return f"{pct_str}%"


def tau_header(threshold: float) -> str:
    return f"{threshold:.0e}".replace("e-0", "e-").replace("e+0", "e+")


def relabel_rows(
    rows: List[Dict[str, object]],
    fieldnames: List[str],
    label_map: Dict[str, str],
) -> Tuple[List[Dict[str, object]], List[str]]:
    labeled_fieldnames = [label_map.get(field, field) for field in fieldnames]
    labeled_rows: List[Dict[str, object]] = []
    for row in rows:
        labeled_rows.append({label_map.get(field, field): row[field] for field in fieldnames})
    return labeled_rows, labeled_fieldnames


def write_excel_sheets(
    output_path: Path,
    tables: Sequence[Tuple[str, List[Dict[str, object]], List[str]]],
) -> None:
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        for sheet_name, rows, fieldnames in tables:
            frame = pd.DataFrame(rows, columns=fieldnames)
            frame.to_excel(writer, sheet_name=sheet_name, index=False)

    wb = load_workbook(output_path)
    text_headers = {"Model", "Label Sampling"}
    epoch_headers = {"Epoch"}
    for ws in wb.worksheets:
        headers = [cell.value for cell in ws[1]]
        for col_idx, header in enumerate(headers, start=1):
            if header in text_headers:
                continue
            for row_idx in range(2, ws.max_row + 1):
                cell = ws.cell(row=row_idx, column=col_idx)
                if cell.value is None:
                    continue
                if header in epoch_headers:
                    if isinstance(cell.value, str):
                        try:
                            cell.value = int(float(cell.value))
                        except ValueError:
                            continue
                    cell.number_format = "0"
        for col_idx, _header in enumerate(headers, start=1):
            max_len = 0
            for row_idx in range(1, ws.max_row + 1):
                value = ws.cell(row=row_idx, column=col_idx).value
                text = "" if value is None else str(value)
                if len(text) > max_len:
                    max_len = len(text)
            ws.column_dimensions[get_column_letter(col_idx)].width = max_len + 2
    wb.save(output_path)


def print_table(title: str, rows: List[Dict[str, object]], fieldnames: List[str]) -> None:
    print(title)
    if not rows:
        print("(empty)")
        print("")
        return

    widths: Dict[str, int] = {}
    for field in fieldnames:
        widths[field] = len(field)
        for row in rows:
            widths[field] = max(widths[field], len(str(row[field])))

    header = " | ".join(f"{field:{widths[field]}}" for field in fieldnames)
    divider = "-+-".join("-" * widths[field] for field in fieldnames)
    print(header)
    print(divider)
    for row in rows:
        print(" | ".join(f"{str(row[field]):{widths[field]}}" for field in fieldnames))
    print("")


def load_label_dataset(project_root: Path, *, sampling_mode: str) -> Tuple[np.ndarray, np.ndarray]:
    device = "cpu"
    txt_filename = str((project_root / "Ellipse.txt").resolve())

    if sampling_mode == "roi-on":
        cfg = config.ROI_RECT_CONFIG
        xy_u, u_fit, *_rest = load_ellipse_uv_eps_fit_rect_dense(
            nx=cfg["nx"],
            ny=cfg["ny"],
            txt_filename=txt_filename,
            device=device,
            ellipse=config.ELLIPSE_PARAMS,
            tau_for_strain=cfg["tau_for_strain"],
            dense_factor=cfg["dense_factor"],
            rect_corners=cfg["rect_corners"],
            target_total=cfg["target_total"],
            random_state=cfg["random_state"],
        )
    elif sampling_mode == "roi-off":
        cfg = config.ROI_OFF_CONFIG
        xy_u, u_fit, *_rest = load_ellipse_uv_eps_fit(
            nx=cfg["nx"],
            ny=cfg["ny"],
            txt_filename=txt_filename,
            device=device,
            ellipse=config.ELLIPSE_PARAMS,
            tau_for_strain=cfg["tau_for_strain"],
            use_dense=cfg["use_dense"],
            dense_factor=cfg["dense_factor"],
            rect_corners=cfg["rect_corners"],
            xlim=cfg.get("xlim"),
            ylim=cfg.get("ylim"),
        )
    elif sampling_mode == "roi-rect":
        cfg = config.ROI_RECT_CONFIG
        xy_u, u_fit, *_rest = load_ellipse_uv_eps_fit_rect_dense(
            nx=cfg["nx"],
            ny=cfg["ny"],
            txt_filename=txt_filename,
            device=device,
            ellipse=config.ELLIPSE_PARAMS,
            tau_for_strain=cfg["tau_for_strain"],
            dense_factor=cfg["dense_factor"],
            rect_corners=cfg["rect_corners"],
            target_total=cfg["target_total"],
            random_state=cfg["random_state"],
        )
    else:
        raise ValueError(f"Unknown sampling mode: {sampling_mode}")

    return xy_u.detach().cpu().numpy().astype(np.float64), u_fit.detach().cpu().numpy().astype(np.float64)


def evaluate_model(npz_path: Path, xy_ref: np.ndarray, u_ref: np.ndarray) -> Dict[str, object]:
    fields = _load_axes_and_fields(npz_path)
    ux_pred = sample_prediction_at_points(fields["ux"], fields["x"], fields["y"], xy_ref)
    uy_pred = sample_prediction_at_points(fields["uy"], fields["x"], fields["y"], xy_ref)
    u_pred = np.concatenate([ux_pred, uy_pred], axis=1)

    ux_true = u_ref[:, 0:1]
    uy_true = u_ref[:, 1:2]
    umag_true = np.sqrt(ux_true ** 2 + uy_true ** 2)
    umag_pred = np.sqrt(ux_pred ** 2 + uy_pred ** 2)

    return {
        "epoch": int(fields["epoch"][0]),
        "E_out": float(fields["E_out"][0]),
        "E_in": float(fields["E_in"][0]),
        "ux_relative_l2": relative_l2(ux_true, ux_pred),
        "uy_relative_l2": relative_l2(uy_true, uy_pred),
        "umag_relative_l2": relative_l2(umag_true, umag_pred),
        "u_relative_l2": relative_l2(u_ref, u_pred),
        "u_pred_points": u_pred,
    }


def load_pde_residual_record(npz_path: Path) -> Dict[str, object]:
    with np.load(npz_path) as zf:
        data = {k: zf[k] for k in zf.files}

    if "pde_residual" not in data:
        raise KeyError(f"{npz_path} missing pde_residual.")

    epoch = -1
    if "epoch" in data:
        epoch_arr = np.asarray(data["epoch"]).reshape(-1)
        if epoch_arr.size > 0:
            epoch = int(epoch_arr[0])
    if epoch < 0:
        epoch = _extract_epoch_from_name(npz_path.name)

    return {
        "epoch": epoch,
        "pde_residual": np.abs(np.asarray(data["pde_residual"], dtype=np.float64)),
    }


def residual_norm_within_threshold_percentages(
    residual_values: np.ndarray,
    thresholds: Sequence[float],
) -> Dict[float, float]:
    residual_norm = np.sqrt(np.abs(np.asarray(residual_values, dtype=np.float64)).reshape(-1))
    return {thr: float(np.mean(residual_norm <= thr) * 100.0) for thr in thresholds}


def main() -> None:
    project_root = Path(__file__).resolve().parent
    default_pimoe_pde = find_latest_phi_snapshot(project_root / "output_roi_on" / "phi_snapshots")
    parser = argparse.ArgumentParser(
        description="Export two Ellipse tables in one workbook."
    )
    parser.add_argument("--txt", type=str, default=str(project_root / "Ellipse.txt"))
    parser.add_argument("--pimoe", type=str, default=str(project_root / "output_roi_on" / "data_output" / "final_fields.npz"))
    parser.add_argument("--apinn", type=str, default=str(project_root / "outputs_apinn" / "final_fields.npz"))
    parser.add_argument("--pinn", type=str, default=str(project_root / "outputs_pinn_single" / "final_fields.npz"))
    parser.add_argument(
        "--pimoe-pde",
        type=str,
        default=str(default_pimoe_pde) if default_pimoe_pde is not None else "",
        help="ADD-PINNs PDE residual snapshot (.npz). Defaults to the latest phi snapshot.",
    )
    parser.add_argument("--csv", type=str, default=str(project_root / "error_summary_u_l2.csv"))
    parser.add_argument(
        "--excel",
        type=str,
        default=str(project_root / "error_summary_tables.xlsx"),
        help="Output Excel workbook path. Different tables are written to different sheets.",
    )
    parser.add_argument(
        "--thresholds",
        type=str,
        default="0.001,0.002,0.005,0.01",
        help="Comma-separated relative-error thresholds, e.g. 0.01,0.02,0.05",
    )
    parser.add_argument(
        "--pde-thresholds",
        type=str,
        default="1e-4,2e-4,5e-4,1e-3",
        help="Comma-separated absolute thresholds tau for |div sigma|.",
    )
    args = parser.parse_args()

    txt_path = Path(args.txt).expanduser().resolve()
    pinn_path = Path(args.pinn).expanduser().resolve()
    apinn_path = Path(args.apinn).expanduser().resolve()
    pimoe_path = Path(args.pimoe).expanduser().resolve()
    if not args.pimoe_pde.strip():
        raise FileNotFoundError("ADD-PINNs PDE snapshot not found. Set --pimoe-pde explicitly.")
    pimoe_pde_path = Path(args.pimoe_pde).expanduser().resolve()
    csv_path = Path(args.csv).expanduser().resolve()
    excel_path = Path(args.excel).expanduser().resolve()
    if excel_path.suffix.lower() != ".xlsx":
        excel_path = excel_path.with_suffix(".xlsx")
    thresholds = [float(item.strip()) for item in args.thresholds.split(",") if item.strip()]
    pde_thresholds = [float(item.strip()) for item in args.pde_thresholds.split(",") if item.strip()]

    for label, path in (
        ("PINN", pinn_path),
        ("APINN", apinn_path),
        ("ADD-PINNs", pimoe_path),
        ("ADD-PINNs PDE", pimoe_pde_path),
        ("Reference", txt_path),
    ):
        if not path.exists():
            raise FileNotFoundError(f"{label} file not found: {path}")

    xy_ref, u_ref = load_reference(txt_path)

    pinn_eval = evaluate_model(pinn_path, xy_ref, u_ref)
    apinn_eval = evaluate_model(apinn_path, xy_ref, u_ref)
    pimoe_eval = evaluate_model(pimoe_path, xy_ref, u_ref)
    pinn_pde = load_pde_residual_record(pinn_path)
    apinn_pde = load_pde_residual_record(apinn_path)
    pimoe_pde = load_pde_residual_record(pimoe_pde_path)

    pinn_sampling_mode = "roi-rect"
    apinn_sampling_mode = "roi-rect"
    pimoe_sampling_mode = infer_sampling_mode_from_path(pimoe_path)

    xy_label_pinn, u_label_pinn = load_label_dataset(project_root, sampling_mode=pinn_sampling_mode)
    xy_label_apinn, u_label_apinn = load_label_dataset(project_root, sampling_mode=apinn_sampling_mode)
    xy_label_pimoe, u_label_pimoe = load_label_dataset(project_root, sampling_mode=pimoe_sampling_mode)

    pinn_fields = _load_axes_and_fields(pinn_path)
    apinn_fields = _load_axes_and_fields(apinn_path)
    pimoe_fields = _load_axes_and_fields(pimoe_path)

    label_pred_pinn = np.concatenate(
        [
            sample_prediction_at_points(pinn_fields["ux"], pinn_fields["x"], pinn_fields["y"], xy_label_pinn),
            sample_prediction_at_points(pinn_fields["uy"], pinn_fields["x"], pinn_fields["y"], xy_label_pinn),
        ],
        axis=1,
    )
    label_pred_apinn = np.concatenate(
        [
            sample_prediction_at_points(apinn_fields["ux"], apinn_fields["x"], apinn_fields["y"], xy_label_apinn),
            sample_prediction_at_points(apinn_fields["uy"], apinn_fields["x"], apinn_fields["y"], xy_label_apinn),
        ],
        axis=1,
    )
    label_pred_pimoe = np.concatenate(
        [
            sample_prediction_at_points(pimoe_fields["ux"], pimoe_fields["x"], pimoe_fields["y"], xy_label_pimoe),
            sample_prediction_at_points(pimoe_fields["uy"], pimoe_fields["x"], pimoe_fields["y"], xy_label_pimoe),
        ],
        axis=1,
    )

    table1_fieldnames = ["Model", "L2 Error on Label Data"] + [threshold_header(thr) for thr in thresholds]
    table2_fieldnames = ["Model"] + [tau_header(thr) for thr in pde_thresholds]

    table1_rows: List[Dict[str, object]] = []
    for model_name, label_true, label_pred, pred_values in (
        ("PINN", u_label_pinn, label_pred_pinn, pinn_eval["u_pred_points"]),
        ("APINN", u_label_apinn, label_pred_apinn, apinn_eval["u_pred_points"]),
        ("ADD-PINNs", u_label_pimoe, label_pred_pimoe, pimoe_eval["u_pred_points"]),
    ):
        row = {
            "Model": model_name,
            "L2 Error on Label Data": format_display(relative_l2(label_true, label_pred)),
        }
        percentages = pointwise_relative_percentages(u_ref, pred_values, thresholds)
        for threshold in thresholds:
            row[threshold_header(threshold)] = format_percent(percentages[threshold])
        table1_rows.append(row)

    table2_rows: List[Dict[str, object]] = []
    for model_name, record in (
        ("PINN", pinn_pde),
        ("APINN", apinn_pde),
        ("ADD-PINNs", pimoe_pde),
    ):
        row = {"Model": model_name}
        percentages = residual_norm_within_threshold_percentages(record["pde_residual"], pde_thresholds)
        for threshold in pde_thresholds:
            row[tau_header(threshold)] = format_percent(percentages[threshold])
        table2_rows.append(row)

    excel_path.parent.mkdir(parents=True, exist_ok=True)
    write_excel_sheets(
        excel_path,
        [
            ("Table1", table1_rows, table1_fieldnames),
            ("Table2", table2_rows, table2_fieldnames),
        ],
    )

    csv_path.parent.mkdir(parents=True, exist_ok=True)
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=table1_fieldnames)
        writer.writeheader()
        writer.writerows(table1_rows)

    print_table("Table 1: label-data L2 error and full-field u pointwise relative-error percentages", table1_rows, table1_fieldnames)
    print_table("Table 2: PDE residual percentages with |div sigma| <= tau", table2_rows, table2_fieldnames)
    print(f"Saved: {csv_path}")
    print(f"Saved: {excel_path}")


if __name__ == "__main__":
    main()
