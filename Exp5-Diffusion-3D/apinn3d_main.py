...
from apinn3d_loss import compute_apinn3d_loss
from apinn3d_model import APINN3D, count_parameters
...
def plot_loss(loss_records: List[List[float]], save_path: Path) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.title("3D APINN Training Loss")
    plt.legend()
    plt.tight_layout()
...
def _prediction_chunks(model: APINN3D, xyz: torch.Tensor, batch_size: int):
...
def evaluate_fields(model: APINN3D, eval_cfg: APINN3DEvalConfig, device: torch.device) -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "g1": g1.detach().cpu().numpy().reshape(n, n, n),
        "g2": g2.detach().cpu().numpy().reshape(n, n, n),
        "gdiff": gdiff.detach().cpu().numpy().reshape(n, n, n),
...
def evaluate_label_u_l2(
    ...
    *,
    ...
    was_training = model.training
    model.eval()
    u_pred_fit, _ = _prediction_chunks(model, xyz_fit, max(1, int(eval_batch_size)))
    if was_training:
        model.train()
...
def save_gate_multiplane(
    ...
    *,
    ...
    r_abs = float(np.max(np.abs(gdiff)))
    if r_abs < 1e-14:
        r_abs = 1e-14

    for row, (_, axis, idx, coord) in enumerate(specs):
        g1_2d = _extract_plane(g1, axis, idx)
        g2_2d = _extract_plane(g2, axis, idx)
        gd_2d = _extract_plane(gdiff, axis, idx)
        extent, xlab, ylab = _plane_extent(axis, x, y, z)
        ...
        axes[row, 0].set_title(f"g1 | {axis}={coord:.3f}")
        axes[row, 0].set_xlabel(xlab)
        axes[row, 0].set_ylabel(ylab)
        fig.colorbar(im0, ax=axes[row, 0], fraction=0.046, pad=0.04)
        ...
        axes[row, 1].set_title(f"g2 | {axis}={coord:.3f}")
        axes[row, 1].set_xlabel(xlab)
        axes[row, 1].set_ylabel(ylab)
        fig.colorbar(im1, ax=axes[row, 1], fraction=0.046, pad=0.04)
        ...
        axes[row, 2].set_title(f"g1-g2 | {axis}={coord:.3f}")
        axes[row, 2].set_xlabel(xlab)
        axes[row, 2].set_ylabel(ylab)
        fig.colorbar(im2, ax=axes[row, 2], fraction=0.046, pad=0.04)
...
def save_periodic_visuals(fields: Dict[str, np.ndarray], output_dir: Path, epoch: int, out_cfg: APINN3DOutputConfig) -> None:
    u_dir = output_dir / out_cfg.u_heatmap_dir
    f_dir = output_dir / out_cfg.f_heatmap_dir
    g_dir = output_dir / out_cfg.gate_heatmap_dir
    ...
    g_png = g_dir / f"gate_heatmap_{tag}.png"
    g_npz = g_dir / f"gate_heatmap_{tag}.npz"
    ...
    save_gate_multiplane(
        ...
        fields["y"],
        fields["z"],
        title_prefix=f"gate fields @ epoch {epoch}",
        save_path=g_png,
    )
...
def pretrain_gate(
    ...
    with torch.no_grad():
        phi0 = phi_signed_c1_sphere(xyz_train)
        if use_hard_target:
    ...
    for ep in range(1, epochs + 1):
        weights, _ = model.gate_weights(xyz_train)
        pred_g1 = weights[:, 0:1]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="3D APINN for single-sphere (c1) interface problem")
...
def _configure_output_dirs(project_root: Path, out_cfg: APINN3DOutputConfig):
...
def main() -> None:
    ...
    data_cfg = APINN3DDataConfig()
    model_cfg = APINN3DModelConfig()
    train_cfg = APINN3DTrainConfig()
    eval_cfg = APINN3DEvalConfig()
    out_cfg = APINN3DOutputConfig()
    ...
    if args.epochs is not None:
        train_epochs = int(args.epochs) - int(train_cfg.gate_pretrain_epochs)
        if train_epochs <= 0:
            raise ValueError("--epochs must be larger than gate_pretrain_epochs for APINN3D")
        blocks = [TrainBlock(epochs=train_epochs, lr=blocks[0].lr)]
    total_epochs = int(sum(max(int(b.epochs), 0) for b in blocks))
    if total_epochs <= 0:
        raise ValueError("total epochs must be positive")
    total_budget = int(train_cfg.gate_pretrain_epochs) + total_epochs
    ...
    pimoe_params = count_parameters(PartitionPINN3D(width=pimoe_cfg.width, depth=pimoe_cfg.depth))
    apinn_params = count_parameters(model)
    print(f"[Model] ADD-PINNs3D params={pimoe_params:,d}, APINN3D params={apinn_params:,d}")
    print(
        f"[Budget] total_epochs={total_budget:,d} "
        f"(gate_pretrain={train_cfg.gate_pretrain_epochs:,d}, train={total_epochs:,d}), "
    ...
    epoch_offset = int(train_cfg.gate_pretrain_epochs)
    ...
    for ep in range(1, total_epochs + 1):
        ...
        if epochs_in_block >= blocks[block_idx].epochs and block_idx + 1 < len(blocks):
            ...
            print(
                ...
                f"lr={new_lr:.3e} at epoch {disp_ep}"
            )

        total, weighted, raw = compute_apinn3d_loss(
            model,
            xyz_int,
            xyz_fit=xyz_fit,
            u_fit=u_fit,
        ...
        if ep % train_cfg.print_every == 0 or ep == 1:
            print(
                f"E{disp_ep:>6d} total={weighted['total'].item():.3e} "
        ...
        if out_cfg.snapshot_every > 0 and ep % out_cfg.snapshot_every == 0:
            ...
            snap_dir = snapshot_root / f"epoch_{disp_ep:07d}"
            save_uf_snapshot(fields_ep, snap_dir, disp_ep)
            save_periodic_visuals(fields_ep, out_dir, disp_ep, out_cfg)
            last_eval_fields = fields_ep
            last_eval_epoch = disp_ep
            print(f"[Save] u/f snapshot: {snap_dir / 'u_f_fields.npz'}")
            print(
                f"[L2] epoch {disp_ep:>6d} | "
                f"u_l2={fields_ep['u_l2'][0]:.3e} (rel={fields_ep['u_rel_l2'][0]:.3e})  "
                f"f_l2={fields_ep['f_l2'][0]:.3e} (rel={fields_ep['f_rel_l2'][0]:.3e})"
            )
            print(
                f"[L2-fit] epoch {disp_ep:>6d} | "
        ...
        if (
            ...
            if new_pts.shape[0] > 0:
                ...
                print(
                    f"[RAR] epoch {disp_ep:>6d} added {new_pts.shape[0]} points "
    ...
    if last_eval_epoch != total_budget:
        fields = evaluate_fields(model, eval_cfg, device)
        final_snap_dir = snapshot_root / f"epoch_{total_budget:07d}"
        save_uf_snapshot(fields, final_snap_dir, total_budget)
        save_periodic_visuals(fields, out_dir, total_budget, out_cfg)
        last_eval_fields = fields
        last_eval_epoch = total_budget
    else:
